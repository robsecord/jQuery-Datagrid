 ...
 *
 * Usage Notes  :
 *  - Data Source can be 1 of 2 types: AJAX or JSON; AJAX = Server-Side, JSON = Client-Side
 *      - For AJAX Data Sources you must provide a URL to your Server-Side Script which will provide the Data.
 *          - Your Script MUST receive the following Parameters as POST vars in the AJAX Request:
 *              - r  = ##                // Number of Rows to return in Response Data
 *              - rs = "abc123" | ##     // Primary Key of the Row to Start at
 *              - sc = "sql field name"  // (optional) Name of the SQL Field to Sort on; should match the Column Model "dataField"
 *              - sd = 1|2               // (optional) The Direction for the Sort: 1 = Ascending, 2 = Descending
 *              - cm = 0|1               // (optional) Request for Column Model; if 1 the Response Data MUST include the Column Model.
 *
 *          - Your Script MUST respond with a well-formatted string of JSON.
 *              - Response Data format:
 *              {
 *                  "resultSet": [
 *                      {
 *                          "index" : ##, // optional; array index of result set
 *                          "cells"  : [
 *                              {"data": "cell data" | ## | [Date Object], "edit": ""},
 *                              ... // additional cells of row
 *                          ],
 *                      },
 *                      ... // additional rows
 *                  ],
 *
 *                  "resultSetInfo": {
 *                      "start"  : true | false  // true if the Result Set is at the starting index (contains the first row)
 *                      "end"    : true | false  // true if the Result Set is at the ending index (contains the last row)
 *                  },
 *
 *                  // optional; only inlcuded in Response Data with Request for Column Model (see POST vars above)
 *                  "columnModel": [
 *                      {
 *                          "label"     : "Column Label",
 *                          "dataType"  : "string | number | date",
 *                          "dataField" : "sql field name" | null,
 *                          "width"     : 100, // in pixels
 *                          "hideable"  : true | false,
 *                          "hidden"    : true | false,
 *                          "sortable"  : true | false,
 *                          "sorted"    : false | 0 | 1 | 2, // false or 0 = Not sorted, 1 = Ascending, 2 = Descending
 ...
 *   - lazy-loading
 *   - add new data types: boolean, currency
 *   - Inline cell editing (with data types)
 *   -
 *   - Bugs:
 *       -
 *       -
...
(function($, undefined) {
    ...
    var
        ...
        defaults = {
            // Data Source
            dataSource          : null,       // Ajax URL or Array of JSON Objects representing Rows matching the Column Model
            dataSourceType      : 'json',     // 'ajax' or 'json'

            // Data Source Range
            //  'rows'       = number of rows to load (ajax only); as more rows are lazy-loaded this may increase up to max.
            //                   this value is used at initial load and on sort.
            //  'maxRows'    = the maximum number of rows to display at one time; lazy loading will delete rows in opposite direction past the max.
            //                   scrolling south past max will delete northerly rows to maintain max, and vice-versa.
            //                   this value is used at initial load, on lazy-load and on sort.
            //  'startIndex' = the primary key or array index of the row to start at; as more rows are lazy-loaded this may change.
            //                   this value is used at initial load and on sort.
            //  'focusIndex' = the primary key or array index of the row to start at; focuses row to middle of datagrid if possible
            //                   this value is used at initial load.
            dataSourceRange     : {'rows': 50, 'maxRows': 200, 'startIndex': 0, 'focusIndex': 0},

            // Column Model
            //  if Data Source Type is AJAX, Column Model should come from AJAX Response
            columnModel         : null,
            ...
            // Lazy Loading
            lazyLoad            : {'north': false, 'south': false}, // false or ## number of items to lazy load (passed to server in ajax request)
    ...
    $.datagrid = function(element, options) {
        ...
        // for Sorting
        this.currentSort = {'column': 0, 'direction': 1, 'field': ''};
    ...
    $dg.fn.extend({
        ...
        initialize: function() {
            ...
            var columnModelSanityCheck = function() {
                for (var i = 0; i < _self.columnModel.length; i++) {
                    ...
                    _self.columnModel[i].width = constrain(_self.columnModel[i].width, _self.options.cellSize.min.width, _self.options.cellSize.max.width);

                    // Check for and fix sortable columns, find sort column
                    if (_self.options.sortable && !!_self.columnModel[i].sortable) {
                        console.log('column['+i+'] sortable');
                        if (!!_self.columnModel[i].sorted) {
                            console.log('column['+i+'] sorted');
                            ...
                            console.log('column['+i+'] not sorted');
                            _self.columnModel[i].sorted = false;
                        }
                    } else {
                        console.log('column['+i+'] not sortable');
                        _self.columnModel[i].sorted = false;
                    }
                }
            };

            // AJAX Data Source/Column Model
            ...
            // JSON Data Source/Column Model
            else {
                // Running as Client-Side Script, disable Server-Side Features
                this.options.sortServer = false;
                this.options.dataSourceRange.rows = this.options.dataSourceRange.maxRows;
                this.options.lazyLoad.north = this.options.lazyLoad.south = false;

                // Perform a Sanity Check on Column Model
                columnModelSanityCheck();
            }
            ...
            this.fetchData({columnModel:true}, function(responseData) {

                // Get Data Source from Response
                _self.dataSource = responseData.dataSource;

                // Limit Size of Result Set
                _self.dataSource.length = Math.min(_self.dataSource.length, _self.options.dataSourceRange.maxRows);

                // Maintain a copy of the Data Source
                _self.dataSourceCopy = $.merge([], _self.dataSource);

                // Get Result Set Info and Column Model from AJAX Data Sources
                if (/ajax/i.test(this.options.dataSourceType)) {

                    // Get Details about current Result Set
                    _self.resultSetInfo = responseData.resultSetInfo;

                    // Get Column Model from Response
                    _self.columnModel = responseData.columnModel;
                ...
                // Sort Data
                if (_self.options.sortable && !_self.options.sortServer) {
                    _self.sortData();
                }
         ...
         * @method fetchData
         * @access private
         * @param Object options An list of options for Fetching Data
        ...
        fetchData: function(options, complete) {
            ...
            if ( ! /ajax/i.test(this.dataSourceType)) {
                ...
                if (typeof complete == 'function') {
                    complete.call(this, {'dataSource': this.dataSource});
                }
                return;
            }
            ...
            // Ajax Data Source
            // Check options
            if (!'rows' in options) {options.rows = this.options.dataSourceRange.rows;}
            if (!'start' in options) {options.start = this.options.dataSourceRange.startIndex;}
            if (!'direction' in options) {options.direction = 1;} // Default to Descending

            // Set Primary Parameters for Request
            params = {'r': options.rows, 'rs': options.start, 'rd': options.direction};

            // Set Optional Parameters for Request
            if ('sortColumn' in options) {params.sc = options.sortColumn;}
            if ('sortDirection' in options) {params.sd = options.sortDirection;}
            if ('columnModel' in options) {params.cm = !!options.columnModel ? 1 : 0;}
            ...
            this.jqXhr = $.ajax({
                ...
                'success': function(responseData) {
                    // Call complete function
                    if (typeof complete == 'function') {
                        complete.call(_self, $.parseJSON(responseData));
                    }
                }
            });
        },
        ...
        prepareDatagrid: function() {
            ...
            if (this.options.lazyLoad.north !== false) {
            ...
            if (this.options.lazyLoad.south !== false) {
        ...
        updateDatagridMetrics: function() {
            ...
            this.lazyLoaderData = {
                'north' : {'height': 0, 'isLoading': false},
                'south' : {'height': 0, 'isLoading': false}
            };
            ...
            // Calculate Height of Lazy Loaders
            if (this.options.lazyLoad.north !== false) { this.lazyLoaderData.north.height = this.$lazyLoaderNorthRow.outerHeight(true); }
            if (this.options.lazyLoad.south !== false) { this.lazyLoaderData.south.height = this.$lazyLoaderSouthRow.outerHeight(true); }

            // Measure start of body table
            this.bodyData.start = this.lazyLoaderData.north.height;
        ...
        updateDatagridEvents: function() {
            ...
            // Watch Scrolling for Lazy Loading
            if (this.options.lazyLoad.north !== false || this.options.lazyLoad.south !== false) {
                this.$container.bind('datagridScrollTop', function(event, scrollY, offsetY) {
                    var northInView = (scrollY == 0),
                        southInView = (scrollY == _self.bodyData.scrollHeight - _self.bodyData.height);

                    // Start Loading more Data if we enter View
                    if (_self.options.lazyLoad.north !== false && northInView) {
                        _self.startLazyLoader('north');
                    }
                    if (_self.options.lazyLoad.south !== false && southInView) {
                        _self.startLazyLoader('south');
                    }

                    // Cancel Loading more Data if we leave View
                    if (_self.options.lazyLoad.north !== false && _self.lazyLoaderData.north.isLoading && !northInView) {
                        _self.cancelLazyLoader('north', false);
                    }
                    if (_self.options.lazyLoad.south !== false && _self.lazyLoaderData.south.isLoading && !southInView) {
                        _self.cancelLazyLoader('south', false);
                    }
                });
            }
        },
        ...
        updateBody: function() {
            ...
            for (i = 0; i < this.dataSource.length; i++) {
                ...
                for (j = 0; j < this.columnOrder.length; j++) {
                    ...
                    if (this.options.spacerCol && j == this.columnOrder.length-1) {
                        ...
                        cellData = this.dataSource[i].cells[idx].data;
            ...
            this.bodyData.scrollHeight += this.lazyLoaderData.north.height + this.lazyLoaderData.south.height;
        ...
        updateLazyLoaderContents: function() {
            ...
            if (this.options.lazyLoad.north !== false && !this.$lazyLoaderNorthMessage) {
            ...
            if (this.options.lazyLoad.south !== false && !this.$lazyLoaderSouthMessage) {
        ...
        updateLazyLoaderEvents: function() {
            ...
            if (this.options.lazyLoad.north !== false) {
                this.$lazyLoaderNorthMessage.find('.cancel-lazy-loader')
                    .bind('click', function(e) {
                        _self.cancelLazyLoader('north', true);
                    });
            }
            ...
            if (this.options.lazyLoad.south !== false) {
                this.$lazyLoaderSouthMessage.find('.cancel-lazy-loader')
                    .bind('click', function(e) {
                        _self.cancelLazyLoader('south', true);
                    });
            }
        },
         ...
         *
         *
         * @method startLazyLoader
         * @access public
         * @return undefined
         */
        startLazyLoader: function(loadDirection) {
            if (this.lazyLoaderData[loadDirection].isLoading) {return}

            // Track Loading State
            this.lazyLoaderData[loadDirection].isLoading = true;



            console.log('lazy load ' + loadDirection);
        },
         ...
         * @return undefined
         */
        cancelLazyLoader: function(loadDirection, requireScroll) {
            // Cancel Loading of Northern Results
            if (/north/i.test(loadDirection)) {
                if (this.options.lazyLoad.north === false || !this.lazyLoaderData.north.isLoading) {return}
                this.lazyLoaderData.north.isLoading = false;
            ...
            // Cancel Loading of Southern Results
                ...
                if (this.options.lazyLoad.south === false || !this.lazyLoaderData.south.isLoading) {return}
                this.lazyLoaderData.south.isLoading = false;
        ...
        scrollTop: function(offsetY) {
            ...
            // Jump into Lazy Loader if within Bounds
            if (this.options.lazyLoad.north !== false && scrollY < this.lazyLoaderData.north.height - 10) {
                scrollY = 0;
            }
            if (this.options.lazyLoad.south !== false && scrollY > this.bodyData.scrollHeight - this.bodyData.height - this.lazyLoaderData.south.height + 10) {
                scrollY = this.bodyData.scrollHeight - this.bodyData.height;
            }
            ...
            // Fire Scroll Event
            this.$container.trigger('datagridScrollTop', [scrollY, offsetY]);
        },
        ...
        scrollLeft: function(offsetX) {
            ...
            // Fire Scroll Event
            this.$container.trigger('datagridScrollLeft', [scrollX, offsetX]);
        },
        ...
        showColumnMenu: function(e, $cell) {
            ...
            // Calculate position for an 'absolute' container
            if (this.$container.css('position') == 'absolute') {
                ...
                // Adjust position to account for container position
                this.columnMenuData.position.top -= containerOffset.top;
                this.columnMenuData.position.left -= containerOffset.left;
            }

            // Calculate position for a 'relative' container
            else {
                containerOffset = this.$container.offset();
                cellOffset = $cell.position();
                ...
                // Adjust position to account for freeze pane and scrolling
                if (!this.columnModel[cellIdx].frozen) {
                    this.columnMenuData.position.left -= this.horzScrollbarData.scrollLeft;
                    this.columnMenuData.position.left += this.headerData.width.freezePane;
                }
            }

            // Get the distance the menu has overflown past the left edge
            //  we do this before constraining the bounds of the menu in order to position the menu caret (arrow)
            leftDiff = Math.min(this.columnMenuData.position.left, 0);
            ...
            $caret.css({'right': this.columnMenuData.caret.position.right - leftDiff + this.options.columnMenuOffset.x});
        ...
        sortCompare: function(valA, valB) {
            var r = 0,
                dir = this.currentSort.direction,
                a = valA.cells[this.currentSort.column].data,
                b = valB.cells[this.currentSort.column].data;
